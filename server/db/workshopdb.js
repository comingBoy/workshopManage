...
module.exports = {
  ...
  async getGroupWorkshop(args) {
    let sql = 'SELECT id,workshopName,checkpointNum,openId,times FROM workshopdb where groupId = ?'
  ...
  async deleteWorkshop(args) {
    let sql = 'DELETE FROM workshopdb where id = ?'
    let params = [args.id]
    let result = await mysqlHelper.query(sql, params)
    return result
  },
  ...
  async getWorkshopInfo(args) {
    let sql = 'SELECT * FROM workshopdb where id = ?'
    let params = [args.workshopId]
    let result = await mysqlHelper.query(sql, params)
    return result
  },
  async getMyWorkshop(args) {
    let sql = 'SELECT id,workshopName,checkpointNum,openId,times FROM workshopdb where groupId = ? AND openId = ?'
